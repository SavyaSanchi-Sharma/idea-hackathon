...
def _ep_from(d):
    lc = d.get("last_commit_date")
    return EndpointRow(
        endpoint_id=d.get("endpoint_id", ""),
        service=d.get("service", ""),
        method=d.get("method", ""),
        path=d.get("path", ""),
        in_registry=bool(d.get("in_registry", False)),
        owner_present=bool(d.get("owner_present", False)),
        owner_team=d.get("owner_team"),
        deprecated_flag=bool(d.get("deprecated_flag", False)),
        auth_scheme=d.get("auth_scheme", "none"),
        runtime=d.get("runtime"),
        runtime_version=d.get("runtime_version"),
        last_commit_date=datetime.fromisoformat(lc.replace("Z", "+00:00"))
            if isinstance(lc, str) and lc else None,
        last_deploy_days=d.get("last_deploy_days"),
        last_seen_days=d.get("last_seen_days"),
        schema_count=int(d.get("schema_count", 0)),
...
def _cls_from(d):
    return ClassificationResult(
        rule_state=d.get("rule_state", "active"),
        rule_reason=d.get("rule_reason", ""),
        ml_state=d.get("ml_state", "active"),
        ml_confidence=float(d.get("ml_confidence", 0.0)),
        agreement=bool(d.get("agreement", False)),
        is_zombie=bool(d.get("is_zombie", False)),
        is_shadow=bool(d.get("is_shadow", False)),
    )
...
def _graph_from(d):
    return GraphFeatures(
        in_routes_to=int(d.get("in_routes_to", 0)),
        out_owned_by=int(d.get("out_owned_by", 0)),
        out_deployed_on=int(d.get("out_deployed_on", 0)),
        blast_radius_total=int(d.get("blast_radius_total", 0)),
        sensitive_db_count=int(d.get("sensitive_db_count", 0)),
        internet_reachable=bool(d.get("internet_reachable", False)),
    )


def _traffic_from(d):
    return TrafficSummary(
        calls_7d=int(d.get("calls_7d", 0)),
        calls_30d=int(d.get("calls_30d", 0)),
        auth_fail_rate_7d=float(d.get("auth_fail_rate_7d", 0.0)),
        p95_latency_ms=float(d.get("p95_latency_ms", 0.0)),
        trend_pct_30d=float(d.get("trend_pct_30d", 0.0)),
    )
...
def build_threat_narrative_json(payload_json: str) -> str:
    p = json.loads(payload_json)
    sp, ctx = build_threat_narrative(
        _ep_from(p["endpoint"]),
        _cls_from(p["classification"]),
        _risk_from(p["risk"]),
        _graph_from(p["graph"]),
        _traffic_from(p["traffic"]),
        list(p.get("owasp_findings") or []),
        _anomaly_from(p.get("anomaly")),
    )
    return json.dumps({"system_prompt": sp, "user_context": ctx})


def build_remediation_playbook_json(payload_json: str) -> str:
    p = json.loads(payload_json)
    sp, ctx = build_remediation_playbook(
        _ep_from(p["endpoint"]),
        _cls_from(p["classification"]),
        _risk_from(p["risk"]),
        _graph_from(p["graph"]),
...
def build_compliance_summary_json(payload_json: str) -> str:
    p = json.loads(payload_json)
    fw_str = p.get("framework", "rbi_2024")
    fw = ComplianceFramework.RBI_2024 if fw_str == "rbi_2024" else ComplianceFramework.PCI_DSS
    sp, ctx = build_compliance_summary(
        _ep_from(p["endpoint"]),
        _cls_from(p["classification"]),
        _risk_from(p["risk"]),
        list(p.get("owasp_findings") or []),
        fw,
        _graph_from(p["graph"]) if "graph" in p else None,
    )
    return json.dumps({"system_prompt": sp, "user_context": ctx})
